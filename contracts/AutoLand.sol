...
contract AutoLand is Ownable, Pausable {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
   ...
   LandTokenStakingV2 public immutable masterchef;
     ...
     * @param _landvault: land vault contract
    ...
    constructor(
        ...
        LandTokenStakingV2 _masterchef,
        address _admin,
        address _treasury
    ) public {
        token = _token;
    ...
    function harvest() external notContract whenNotPaused {
       LandTokenStakingV2(masterchef).withdraw(0);
    ...
    function emergencyWithdraw() external onlyAdmin {
        LandTokenStakingV2(masterchef).emergencyWithdraw(0);
    }
    ...
    function calculateHarvestCakeRewards() external view returns (uint256) {
        uint256 amount = LandTokenStakingV2(masterchef).pendingCake(0, address(this));
    ...
    function calculateTotalPendingCakeRewards() external view returns (uint256) {
        uint256 amount = LandTokenStakingV2(masterchef).pendingReward(address(this));
        amount = amount.add(available());
    ...
    function withdraw(uint256 _shares) public notContract {
        ...
        if (bal < currentAmount) {
            ...
            LandTokenStakingV2(masterchef).withdraw(balWithdraw);
    ...
    function balanceOf() public view returns (uint256) {
        (uint256 amount, ) = LandTokenStakingV2(masterchef).userInfo(address(this));
    ...
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            LandTokenStakingV2(masterchef).deposit(bal);
        }
    }
